...
# once we leave the block, the varibles defined inside the block cannot be seen because they have an
# inner scope

# The object ideas will change as the variables are reassigned and given new values.
...
# We simply took the contents of the block from the previous exercise and moved it
# to a method, to which we are passing all of our outer variables.
...
# The objects id's before and after the method will not change because methods have their own scope
# Trying to access the variables declared inside a different method will not work
...
# using += creates a new string object that is assigned to a_strng_param.
# the original string is not mutated in any way
puts "My array looks like this now: #{my_array}"
# This will display "My array looks like this now: ["pumpkins", "rutabaga"]"
# This is because << mutates the array.
...
# << mutates my_string
puts "My array looks like this now: #{my_array}"
# my_array remains unchanged by tricky_method_two because
# an_array_param = ['pumpkins', 'rutabaga'] creates a new array object
...
# The below method works the same as the one above it
def color_valid(color)
